...
async function checkRateLimit(
  ...
  if (count !== null && count >= maxCalls) return false // exceeded
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Rate limit: 20 queries per 60 seconds per user
    ...
    if (!Array.isArray(matches) || matches.length === 0) {
      return new Response(
        JSON.stringify({ answer: 'I don\'t know based on this document.', sources: [] }),
        { headers: { ...headers, 'Content-Type': 'application/json' } },
      )
    }

    const contextParts = matches.map(
      (m: { content: string; chunk_index: number }) => `[${m.chunk_index}] ${m.content}`,
    )
    const context = contextParts.join('\n\n')

    // Step 3: Build system prompt with prompt-injection guard
    const systemPrompt =
    ...
    const sources = matches.map((m: { content: string; chunk_index: number }) => ({
